...
class PdoGsb {
    ...
    /**
     * 
     * @return un tableau avec les fiches Frais Valides
     */
    public function getFicheFraisValide() {
        $req = PdoGsb::$monPdo->prepare('SELECT mois, nom, prenom FROM fichefrais inner join visiteur on fichefrais.idVisiteur = visiteur.id WHERE idEtat = "VA" ORDER BY mois');
        $req->execute();
        $fiche = $req->fetchAll();
        return $fiche;
    }    
    ...
    /**
     * Retourne le montant total des frais hors forfait
     * @param $idVisiteur id du visiteur
     * @param $mois sous la forme aaamm
     */
    ...
    /**
     * MAJ le montant total des frais en BDD
     * @param $idVisiteur id du visiteur
     * @param $mois sous la forme aaamm
     * @param $montant sous la forme décimale
     */
